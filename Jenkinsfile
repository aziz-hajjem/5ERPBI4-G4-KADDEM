pipeline {
    ...
    stages {
        stage('Checkout Git repository') {
            steps {
                ...
                git branch: 'FirasZarrouk-5ERPBI4-G4', url: 'https://github.com/aziz-hajjem/5ERPBI4-G4-KADDEM.git'
            }
        }
           ...
           steps {
               sh '''
                   mvn sonar:sonar \
                       ...
                       -Dsonar.login=admin \
                       -Dsonar.password=Firas123456789? \
        ...
        stage('Build Docker Image') {
            steps {
                script {
                    ...
                    def dockerImage = docker.build("firas20/kaddem:0.0.1")
                }
            }
        }
        ...
        stage('Deploy Image to DockerHub') {
            steps {
                script {
                    ...
                    sh 'docker login -u firas20 -p firas123456789'
                    sh 'docker push firas20/kaddem:0.0.1'
                }
            }
        }
   ...
   post {
        success {
            mail to: 'firas.zarrouk@esprit.tn',
